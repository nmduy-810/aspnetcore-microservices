...
public class LoggingDelegatingHandler : DelegatingHandler
{
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                _logger.LogInformation("Receive a success response from {Url}", response.RequestMessage?.RequestUri);
            }
            else
            {
            ...
            when (e.InnerException is SocketException {SocketErrorCode: SocketError.ConnectionRefused})
        {
            if (request.RequestUri != null)
            {
                ...
                _logger.LogCritical(e, "Unable to connect to {Host}. Please check the configuration/settings to ensure the " +
                                       "correct URL for the service has been configured", hostWithPort);
            }
        }
