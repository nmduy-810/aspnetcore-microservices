...
public static class CustomerContextSeed
{
    ...
    {
        var customer =
            await customerContext.Customers.SingleOrDefaultAsync(x =>
                x.UserName.Equals(username) || x.EmailAddress.Equals(email));

        if (customer == null)
        {
            var newCustomer = new Entities.CatalogCustomer()
            {
                UserName = username,
                FirstName = firstname,
                LastName = lastName,
