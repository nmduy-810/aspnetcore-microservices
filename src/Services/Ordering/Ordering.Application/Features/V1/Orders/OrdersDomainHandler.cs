...
public class OrdersDomainHandler : INotificationHandler<OrderCreatedEvent>, INotificationHandler<OrderDeletedEvent>
{
    ...
    public Task Handle(OrderCreatedEvent notification, CancellationToken cancellationToken)
    {
        ...
        var emailRequest = new MailRequest
        {
            ...
            Subject = $"Hello {notification.FirstName}, your order was created"
        };
        
        try
        {
            _emailService.SendEmailAsync(emailRequest, cancellationToken);
            _logger.Information("Sent Created Order to email {NotificationEmailAddress}", notification.EmailAddress);
        }
        catch (Exception ex)
        {
            _logger.Error("Order {NotificationId} failed due to an error with the email service: {ExMessage}", 
