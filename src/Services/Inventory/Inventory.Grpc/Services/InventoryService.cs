...
public class InventoryService : StockProtoService.StockProtoServiceBase
{
    ...
    {
        _logger.Information("BEGIN Get Stock of ItemNo: {RequestItemNo}", request.ItemNo);
        var stockQuantity = await _repository.GetStockQuantity(request.ItemNo);
        var result = new StockModel()
        {
            Quantity = stockQuantity
        };
        _logger.Information("END Get Stock of ItemNo: {RequestItemNo} - Quantity: {Quantity}", request.ItemNo, result.Quantity);
        return result;
    }
}