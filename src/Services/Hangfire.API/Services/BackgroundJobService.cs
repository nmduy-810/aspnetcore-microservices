...
public class BackgroundJobService : IBackgroundJobService
{
    ...
    public string? SendEmailContent(string email, string subject, string emailContent, DateTimeOffset enqueueAt)
    {
        ...
        try
        {
            ...
            _logger.Information("Sent email to {Email} with subject: {Subject} - Job Id: {JobId}", email, subject, jobId);

            return jobId;
        }
        catch (Exception ex)
        {
            _logger.Error("failed due to an error with the email service: {ExMessage}", ex.Message);
        }
